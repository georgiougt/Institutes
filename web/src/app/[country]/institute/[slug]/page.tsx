...
async function getInstitute(idOrSlug: string) {
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 8000);

    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api/v1'}/institutes/${idOrSlug}`, { 
      cache: 'no-store',
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; country: string }> }): Promise<Metadata> {
  ...
  const institute = await getInstitute(resolvedParams.slug);
  if (!institute) return {};
      ...
      <main className="flex-1 bg-white pt-8 pb-20">
        <div className="container mx-auto max-w-[1100px] px-6 grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            {/* Reviews Section */}
            <section id="reviews">
              <ReviewSection instituteId={institute.id} />
            </section>
          </div>
